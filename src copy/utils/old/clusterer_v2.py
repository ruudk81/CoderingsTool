...
class RepresentationEngine:
    ...
    Wraps the existing CTfidfRepresentation module from development/representation/.
    """
    ...
    def _ensure_ctfidf(self):
        ...
        if self._ctfidf is None:
            try:
                ...
                development_path = os.path.join(parent_path, 'development')
                if development_path not in sys.path:
    ...
    for result in cluster_results:
        ...
        if result.response_ideas:
            for idea_submodel in result.response_ideas:
                ...
                # Create new ClusterSubmodel with cleaned text, preserving all inherited fields
                cleaned_submodel = models.ClusterSubmodel(
                    idea_id=idea_submodel.idea_id,
                    idea=cleaned_idea,
                    taxonomy_phrase=getattr(idea_submodel, 'taxonomy_phrase', '') or '',
                    parent_category=getattr(idea_submodel, 'parent_category', '') or '',
                    sentiment=getattr(idea_submodel, 'sentiment', 'neutral') or 'neutral',
                    sense=getattr(idea_submodel, 'sense', 'factual') or 'factual',
