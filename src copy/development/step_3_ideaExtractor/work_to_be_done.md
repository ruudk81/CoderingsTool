...
Known example: ATTRIBUTES_ASSOCIATIONS has `domain_diagnostic = "What entity has the trait?"` — this fails for single-entity brand surveys where the entity is always the same (e.g., Merk X). The LLM falls back to topic-based domains ("financiële aspecten", "marketing en communicatie") instead of orthogonal attribute types.
...
**Status:** Needs validation by re-running step 3 on Merk X dataset.
