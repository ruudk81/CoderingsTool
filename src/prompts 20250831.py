...
Your task is to identify and construct *themes* from descriptive codes that have been grouped together around one central concept by HDBSCAN.

<conceptual_guidance>
- A theme is NOT: a list of responses, a single descriptive label, a speculation beyond the data, or a mixture of unrelated/conflicting ideas.
- A good theme expresses a clear analytic idea that explains how and why the responses belong together.
- Themes must be atomic and conceptually unified. If more than one concept appears, split them into separate themes.
- Each theme must be distinct from other themes.
- If a theme cannot be explained in terms of a single central organizing concept (COC) and shared meaning, it is not a theme.
</conceptual_guidance>

Here is the research question you are working with:
<research_question>
{survey_question}
</research_question>

Here is the cluster with descriptive codes for you to analyze:
<descriptive_codes>
{cluster_text}
</descriptive_codes>

Follow these steps to conduct your thematic analysis:

1. Examine <descriptive_codes> in light of the <research_question>.
2. Find coherent patterns of shared meaning across multiple codes (ignore one-off mentions).
3. Focus on the broadest shared meaning that conveys a clear analytic idea.
4. Identify the central organizing concept (COC) that underpins this idea.
5. Aim for the fewest possible number of themes based on this COC.
6. Only if one COC cannot cover all codes, split them into separate themes. 
7. Create a label for each theme you constructed by adhering to these rules:
	- Must be ≤10 words
	- A single unifying analytic idea
	- Expressed as a noun phrase that includes the COC
8. Write a description for each label:
	- Must be ≤30 words
	- States the strongest shared pattern of meaning
	- Clarifies this meaning in light of the research question
...
[  ] No actor mentioned in descriptopn
[  ] No repetition of survey subject in description


Your output should be a valid JSON array with one or more theme objects. Each object should have this structure:
[
  {{
  "theme_id": [number],
  "theme_name": "[theme label in ≤10 words, in {language}]",
  "theme_statement": "[label description in ≤30 words, in {language}]"
     }}
 ]
...
After your analysis, provide your final output as a JSON array.
"""
