...
RUN_UNTIL_STEP = 7          # 0-7 (inclusive)
...
def run_pipeline(
    ...
    if run_until_step >= 0:
        print(f"\n{'='*70}\nStep 0 — {STEP_NAMES[0]}\n{'='*70}")
        from steps.step_0_dataLoader.run_dataLoader import run_step as run_step_0, StepConfig as Step0Config
    ...
    if run_until_step >= 1:
        print(f"\n{'='*70}\nStep 1 — {STEP_NAMES[1]}\n{'='*70}")
        from steps.step_1_preProcessor.run_preProcessor import run_step as run_step_1, StepConfig as Step1Config
    ...
    if run_until_step >= 2:
        print(f"\n{'='*70}\nStep 2 — {STEP_NAMES[2]}\n{'='*70}")
        from steps.step_2_qualityFilter.run_qualityFilter import run_step as run_step_2, StepConfig as Step2Config
    ...
    if run_until_step >= 3:
        print(f"\n{'='*70}\nStep 3 — {STEP_NAMES[3]}\n{'='*70}")
        from steps.step_3_ideaExtractor.run_ideaExtractor import run_step as run_step_3, StepConfig as Step3Config
    ...
    if run_until_step >= 4:
        print(f"\n{'='*70}\nStep 4 — {STEP_NAMES[4]}\n{'='*70}")
        from steps.step_4_classifier.run_classifier import run_taxonomy
        run_taxonomy()

    # --- Step 5: Codebook Generation ---
    if run_until_step >= 5:
        print(f"\n{'='*70}\nStep 5 — {STEP_NAMES[5]}\n{'='*70}")
        from steps.step_5_codeGenerator.run_codeGenerator import run_codebook
        run_codebook()

    # --- Step 6: Code Assignment ---
    if run_until_step >= 6:
        print(f"\n{'='*70}\nStep 6 — {STEP_NAMES[6]}\n{'='*70}")
        from steps.step_6_codeAssigner.run_codeAssigner import run_assignment
        run_assignment()

    # --- Step 7: Export ---
    if run_until_step >= 7:
        print(f"\n{'='*70}\nStep 7 — {STEP_NAMES[7]}\n{'='*70}")
        from steps.step_7_export.run_export import run_step as run_step_7, StepConfig as Step7Config
