...
def show_upload_page():
    ...
    if st.session_state.available_variables:
        ...
        if st.session_state.variable_preview is not None:
            ...
            var_for_label = (st.session_state.selected_variables[0]
                             if st.session_state.get('is_merged_variable') and st.session_state.get('selected_variables')
                             else st.session_state.selected_variable)
...
def show_preprocessing_page():
    ...
    st.header("Stap 1: Tekstverwerking" if lang == "nl" else "Step 2: Text preprocessing")
    ...
    #------------------
    if is_step_completed(2):  # Step 2 = preprocessing
    ...
    #------------------------
    #state = BEFORE processing (step 2 not completed yet)
    #------------------------
    else:
        # Show preprocessing button
...
def show_filtering_page():
    ...
    if st.session_state.get('waiting_for_continue_filtering', False):
        ...
        if st.session_state.get('selected_variable') and st.session_state.get('selected_id_column'):
           ...
           sample_size = f"{stats.get('total_with_codes', 0) + stats.get('total_without_codes', 0)}" if stats else f"{st.session_state.selected_sample_size}"
