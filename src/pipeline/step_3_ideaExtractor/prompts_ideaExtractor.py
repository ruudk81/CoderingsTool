...
NO residual catch-all: a domain must be defined by what it contains, never by what the other domains do not. If a domain's boundary_test can only be stated as "does not belong to any of the others", it is a catch-all — split it along sharper axes or drop it. A domain that groups characteristics of the entity itself is legitimate, provided its boundary_test states positively what kind of characteristic it names. EXCEPTION: a clean, well-defined "no/weak association" domain IS allowed when many responses genuinely express the absence of any association — this is a real response type, not a catch-all.
