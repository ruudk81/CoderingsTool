...
def build_domain_discovery_prompt(
    *,
    ...
    builder. Note that it deliberately does NOT show the standing domains: telling
    discovery they exist, so it would stop proposing its own "no association"
    domain, was measured over three runs and made the partition markedly less
    stable — see dev/WORK.md, "the discovered no-content domain is load-bearing".
    """
...
- NO broad evaluative catch-all: do not keep or create a vague impression bucket such as "general impression", "overall reputation", or "character" that mixes many unrelated qualities. If one domain would absorb a large share of responses, split it along sharper subject axes. Answers expressing the absence of any association are already covered by the first fixed domain — do not give them a domain of their own.
...
- DESCRIPTIVE DOMAINS ONLY: every domain you return names a DESCRIPTIVE subject/aspect — never a sentiment or judgment. Even if all its ideas are positive or negative, the domain describes WHAT is referred to, not how good or bad it is; direction (positive/negative) is captured separately by valence, never by domains. Reframe any evaluative bucket (e.g. "reputation/appreciation", "good vs bad") descriptively as the subject being judged.
