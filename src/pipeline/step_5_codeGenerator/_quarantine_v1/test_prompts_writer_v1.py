...
from pipeline.step_5_codeGenerator._quarantine_v1.prompts_writer_v1 import (
    build_writer_prompt_v1,
)
from pipeline.step_5_codeGenerator.prompts_writer import CodeText, make_writer_model
...
def test_prompt_states_the_fixed_number_of_codes():
    ...
    prompt = build_writer_prompt_v1(shapes, {}, "Does the response say that...", "nl-NL")
    assert "2" in prompt
...
def test_prompt_forbids_claiming_a_neighbours_territory():
    ...
    prompt = build_writer_prompt_v1(shapes, {}, "stem", "nl-NL")
...
def test_prompt_shows_attribute_names_not_ids():
    ...
    prompt = build_writer_prompt_v1(shapes, concept_by_id, "stem", "nl-NL")
...
def test_prompt_order_is_stable_across_calls():
    ...
    first = build_writer_prompt_v1(shapes, concept_by_id, "stem", "nl-NL")
    second = build_writer_prompt_v1(shapes, concept_by_id, "stem", "nl-NL")
    assert first == second


def test_prompt_uses_the_dimension_diagnostic_stem():
    shapes = [shape("K1", "positive", "prijs", ["A1"])]
    prompt = build_writer_prompt_v1(shapes, {}, "Does the response evaluate the price?", "nl-NL")
    assert "Does the response evaluate the price?" in prompt


def test_prompt_ends_with_the_instructor_hint():
    prompt = build_writer_prompt_v1([shape("K1", "positive", "prijs", ["A1"])], {}, "stem", "nl-NL")
...
def test_prompt_veto_rule_is_present():
    prompt = build_writer_prompt_v1([shape("K1", "neutral", "u", ["A1", "A2"], origin="pooled")], {}, "stem", "nl-NL")
...
def test_prompt_without_taken_names_mentions_none():
    shapes = [shape("K1", "positive", "prijs", ["A1"])]
    prompt = build_writer_prompt_v1(shapes, {}, "stem", "nl-NL", taken_names=None)
    assert "off limits" not in prompt.lower()

    prompt_empty = build_writer_prompt_v1(shapes, {}, "stem", "nl-NL", taken_names=[])
