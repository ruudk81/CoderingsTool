...
# Absolute imports, geen relatieve — dit bestand moet met een enkele klik
# (VS Code Code Runner, `python run_experiment.py`) blijven draaien, en dan is
# `__package__` leeg: `from .analysis import ...` zou daar stuklopen op
# "attempted relative import with no known parent package". Vandaar de
# `sys.path`-kop in plaats van een package-relatieve import.
...
async def collect(config_name: str, runs: int, set_index: int, salted: bool = True) -> Path:
    ...
    config = ConsensusConfig(model_relations=spec["model"])

    original_effort = project_config.STEP_EFFORT.get("codegen_relations")
    project_config.STEP_EFFORT["codegen_relations"] = spec["effort"]
    try:
        salts = [f"set{set_index}run{i}" if salted else "" for i in range(runs)]
        proposals, mislukt = await resolve_consolidations(
        ...
        partitions = [[tuple(sorted(g.member_ids))
                      ...
                      for p in proposals]
        if mislukt:
            print(f"  LET OP: {mislukt} van de {runs} runs kwam niet terug")
    finally:
    ...
    print(f"\n{len(partitions)} van de {runs} runs weggeschreven naar {path}")
    return path
