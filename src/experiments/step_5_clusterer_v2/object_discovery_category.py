...
"""
MECE Object Discovery from Categories

Clusters unique ontology categories to discover MECE objects using the
Clusterer pipeline (HDBSCAN → Phase B object labeling → Phase C MECE consolidation).
...
Usage:
    ...
    Or: cd src && python -m experiments.step_5_clusterer_v2.object_discovery_category
"""
...
# Examples: "{category}", "{root} -> {category}", "{node}", "{template_prefix}{node}"
...
#%% Cell 3: Run Clusterer with OBJECT prompts
...
clusterer = Clusterer(
    ...
    mece_prompt_template=MECE_OBJECT_CONSOLIDATION_PROMPT,
...
print_discovery_results(clusterer, item_names, item_metadata, "OBJECT")
