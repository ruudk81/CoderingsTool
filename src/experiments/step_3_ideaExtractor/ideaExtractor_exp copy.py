...
4. Live warm-up with one-shot calibration from production data
"""
...
BOOTSTRAP_TIMEOUT_SECONDS = DEFAULT_TIMEOUT_CONFIG.bootstrap_timeout_seconds  # Unused: bootstrap phase removed
...
# Bootstrap settings
BOOTSTRAP_NUM_PROBES = DEFAULT_BOOTSTRAP_CONFIG.num_probes  # Unused: bootstrap phase removed
...
class DynamicSemaphore:
    """Semaphore that supports increasing concurrency at runtime.

    Works as a drop-in replacement for asyncio.Semaphore (async with compatible).
    Only supports increasing the limit — shrinking would require complex tracking
    and is not needed since we primarily correct upward as production latency data
    replaces the config-default estimate.
    """
    ...
    def resize(self, new_limit: int) -> int:
        """Increase concurrency limit. Returns the delta (0 if no change or shrink requested)."""
...
async def bootstrap_measure_async(call_fn, n_probes: int = 3):
    """DEPRECATED: Retained for reference. Bootstrap phase removed in favor of bootstrapless startup.
...
{cats_text}"""
    ...
    def _initialize_rate_limiters(self, avg_latency_s: float, avg_tokens: int, limits, num_tasks: int) -> int:
        ...
        self.semaphore = DynamicSemaphore(min(num_tasks, optimal))
        self.optimal_concurrency = min(num_tasks, optimal)

        # Reinitialize token bucket with calibrated capacity
    ...
    def _initialize_warm_up_rate_limiters(self, limits: 'RateLimits', num_tasks: int) -> int:
        """Initialize conservative rate limiters for the warm-up phase.

        Uses fixed low concurrency and reduced headroom to safely measure
        production latency and token usage before scaling up.
        """
        warm_up_conc = min(self.warm_up_config.concurrency, num_tasks)

        # Conservative arrival rate: 50% of normal headroom
        arrival_rate = min(
            limits.requests_per_minute * self.warm_up_config.arrival_fraction / 60,
            limits.tokens_per_minute * self.warm_up_config.arrival_fraction / max(self.avg_tokens, 1) / 60
        )
        arrival_rate = max(arrival_rate, 0.5)  # Floor: at least 0.5 req/s

        self.rate_limiter = AsyncLimiter(1, time_period=1.0 / arrival_rate)
        self.semaphore = DynamicSemaphore(warm_up_conc)
        self.optimal_concurrency = warm_up_conc

        # Token bucket at full rate (it self-regulates via acquire/wait)
        self.tpm_bucket = TokenBucket(int(limits.tokens_per_minute * self.processing_config.rate_limit_headroom))

        # Track initial arrival rate for PID
        self.current_arrival_rate = arrival_rate
    ...
    def _calibrate_from_warm_up(self, num_tasks: int) -> None:
        """One-shot calibration: measure production data, recalculate, swap all limiters.

        Fires once after enough warm-up completions. Reads median latency and
        mean token usage from production data, then reinitializes all three
        rate limiting layers (semaphore, token bucket, arrival rate) with
        calibrated values. PID controller is reset for a clean start.
        """
        # Measure from production data
        ...
        old_concurrency = self.optimal_concurrency
        self.avg_tokens = measured_avg_tokens
        self.bootstrap_avg_tokens = measured_avg_tokens  # For end-of-run reporting

        # Recalculate optimal concurrency via Little's Law and swap all limiters
        limits = self.rate_limits
        self._initialize_rate_limiters(measured_latency, measured_avg_tokens, limits, num_tasks)

        # Reset PID controller (fresh start with calibrated rates)
        self.pid_controller.reset()

        # Log calibration
        print(f"\n{'='*60}")
        print(f"WARM-UP CALIBRATION (from {len(self.actual_total_tokens)} samples)")
        print(f"   Latency: {DEFAULT_LATENCY_SECONDS}s (default) -> {measured_latency:.1f}s (measured)")
        print(f"   avg_tokens: {old_avg} (tiktoken) -> {measured_avg_tokens} (measured)")
        print(f"   Concurrency: {old_concurrency} (warm-up) -> {self.optimal_concurrency} (calibrated)")
        print(f"   Arrival rate: {self.current_arrival_rate:.1f}/s")
        print(f"{'='*60}")
    ...
    def _adjust_concurrency_if_needed(self, num_tasks: int) -> bool:
        """Adjust semaphore concurrency based on measured production latency.

        Initial concurrency is calculated from config defaults (DEFAULT_LATENCY_SECONDS).
        Once we have enough production latency samples, recalculate concurrency
        using Little's Law with actual measured latency and resize the semaphore.

        Only increases concurrency (never shrinks) to avoid stalling in-flight requests.
        """
        if self.current_arrival_rate is None or not self.latency_tracker.values:
            return False

        # Need enough production samples (not just bootstrap seeds)
        if len(self.latency_tracker.values) < 20:
            return False

        # Use median latency (more stable than EMA for concurrency planning)
        measured_latency = float(np.median(list(self.latency_tracker.values)))

        # Little's Law: concurrency = arrival_rate × latency
        needed = int(self.current_arrival_rate * measured_latency)
        max_concurrency = self.processing_config.concurrency_cap_default
        target = min(max_concurrency, max(needed, self.optimal_concurrency), num_tasks)

        if target <= self.optimal_concurrency:
            return False

        old = self.optimal_concurrency
        delta = self.semaphore.resize(target)
        if delta > 0:
            self.optimal_concurrency = target
            if self.verbose_reporter.enabled:
                self.verbose_reporter.stat_line(
                    f"Concurrency adjusted: {old} → {target} "
                    f"(measured latency={measured_latency:.1f}s, arrival_rate={self.current_arrival_rate:.1f}/s)"
                )
            return True
        return False
    ...
    def _adjust_throughput_if_needed(self) -> bool:
        ...
        # Calculate ratio of actual to current estimate
        ratio = actual_avg / current_avg if current_avg > 0 else 1.0

        # Only trigger threshold adjustment for significant underestimation
        ...
        old_tpm_throughput = self.rate_limits.tokens_per_minute * self.processing_config.rate_limit_headroom / current_avg / 60
    ...
    async def process_all_tasks_async(self, tasks: List[Dict]) -> List[models.IdeasExtractedModel]:
        ...
        # === PHASE 5: WARM-UP INITIALIZATION ===
        # Start with conservative concurrency, calibrate from production data.
        # See probe_call_no_structured() and bootstrap_measure_async() for legacy reference.
        avg_latency_s = DEFAULT_LATENCY_SECONDS    # Initial estimate for reporting
        avg_tokens = self.avg_tokens               # Phase 4 tiktoken calculation
        self.bootstrap_avg_tokens = self.avg_tokens

        warm_up_conc = self._initialize_warm_up_rate_limiters(limits, len(tasks))
        ...
        if self.verbose_reporter.enabled:
            self.verbose_reporter.stat_line(f"\nWarm-up: {warm_up_conc} slots, calibration after {warm_up_samples} completions")
            self.verbose_reporter.stat_line("Dynamic correction (PID + concurrency) continues after calibration")

        # === PHASE 6: Print setup info and launch workers ===
        # Compute post-calibration estimates for reporting and worker sizing
        api_limits = ApiLimits(limits.tokens_per_minute, limits.requests_per_minute)
        post_cal_concurrency = compute_optimal_concurrency(
            api_limits, avg_latency_s, avg_tokens,
            processing_config=self.processing_config,
            cap=self.processing_config.concurrency_cap_default,
            min_conc=self.processing_config.concurrency_min_permissive
        )
        ...
        print("\nRATE LIMITING SETUP - Live warm-up -> auto-calibration")
        ...
        print(f"- Expected throughput (post-calibration): {expected_throughput:.1f}/s ({bottleneck} limited)")
        print(f"- Warm-up: {warm_up_conc} slots, calibrate after {warm_up_samples} completions")
        print(f"- Post-calibration concurrency estimate: {post_cal_concurrency}")
        print(f"- Processing {len(tasks):,} tasks")

        # Calculate predicted utilization stats (post-calibration estimates)
        predicted_rpm_usage = expected_throughput * 60
        predicted_tpm_usage = expected_throughput * self.avg_tokens * 60
        ...
        print(f"\nPREDICTED UTILIZATION (post-calibration)")
        ...
        # Workers: plan for post-calibration scale, semaphore limits active during warm-up
        max_workers = self.processing_config.max_workers if hasattr(self.processing_config, 'max_workers') else 1000
        adaptive_min_workers = max(10, post_cal_concurrency * 2)
        num_workers = min(max_workers, max(adaptive_min_workers, int(expected_throughput * avg_latency_s * 2.0)), len(tasks))

        print(f"\nWorkers launched (concurrent subroutines): {num_workers}")
        print(f"API calls in flight (concurrency ceiling/semaphore): {self.optimal_concurrency} [warm-up]")
        ...
        while not queue.empty():
            ...
            if now - last_report >= PROGRESS_REPORT_INTERVAL:
                ...
                phase_tag = "" if self._warm_up_calibrated else " [warm-up]"
                print(f"Progress: {completed}/{len(tasks)} ({completed/len(tasks)*100:.1f}%), "
                      f"Rate: {rate:.1f}/s, Queue: {remaining}{phase_tag}")
                last_report = now

            # One-shot calibration after warm-up (use actual completions with token data, not dispatched count)
            ...
            if now - last_adjustment >= ADJUSTMENT_INTERVAL:
                ...
                # Adjust concurrency based on measured production latency
                self._adjust_concurrency_if_needed(len(tasks))
                last_adjustment = now
            ...
            if self.verbose_reporter.enabled and now - last_diagnostics >= DIAGNOSTIC_INTERVAL:
                ...
                if token_stats['status'] == 'learning' and token_stats['actual_samples'] >= 10:
                    ...
                    if adj_total > 0:
                        self.verbose_reporter.stat_line(f"Token usage: Initial {initial_avg:.0f}, Adjusted {current_avg:.0f}, Actual {actual_avg:.0f} "
        ...
        if self._warm_up_calibrated:
            print(f"- Warm-up calibration: after {self._warm_up_target_samples} samples")
        if self.v3_stats['threshold_adjustments'] > 0:
            ...
            print(f"  - Initial avg_tokens (tiktoken): {self.bootstrap_avg_tokens}, Final avg_tokens: {self.avg_tokens}")
