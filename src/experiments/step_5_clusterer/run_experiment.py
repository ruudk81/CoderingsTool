...
This experiment runner uses LOCAL COPIES of the clusterer that can be modified
without affecting the production pipeline. Edit these files for experimentation:
...
- prompts_exp.py               (LLM prompts for cluster labeling)

Dataset configuration is centralized in experiments/test_data.py.

Usage:
    cd src && python -m experiments.step_5_clusterer.run_experiment

Toggle modes:
    USE_EXPERIMENTAL_CLUSTERER = True  -> Uses LOCAL experimental copies (safe to modify)
    USE_EXPERIMENTAL_CLUSTERER = False -> Uses PRODUCTION clusterer (src/utils/clusterer.py)
"""
...
# =============================================================================
# TOGGLE: Use local experimental copies or production clusterer
# =============================================================================
...
if USE_EXPERIMENTAL_CLUSTERER:
    # Use LOCAL experimental copies (safe to modify without affecting production)
    try:
