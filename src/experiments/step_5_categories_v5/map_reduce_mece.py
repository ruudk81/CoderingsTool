"""
Map-Reduce MECE engine for Category Discovery V5.

Per-partition category extraction via 3-step LLM pipeline with partition
...
Pipeline for each partition:
  1. MAP:    Batch labels, identify candidate categories per batch
  2. REDUCE: Consolidate categories across batches (skipped for single-batch)
  3. MECE:   Apply MECE constraints with boundary criteria

Key differences from V4:
  - "object" → "partition", "topic/theme/COC" → "category"
...
Adapted from V4's ObjectAwareMapReduceMECE.
"""
...
class MapReduceMECE:
    """
    Per-partition Map-Reduce MECE category extractor with partition context.

    All partitions are processed concurrently. All LLM calls share a single
    async client, semaphore, and rate limiter for efficient throughput.

    Three-step pipeline for each partition:
    1. MAP:    Batch labels, identify candidate categories (async)
    2. REDUCE: Consolidate categories across batches (async)
    ...
    def process_all_partitions(
        ...
        Args:
            ...
            taxonomy_axis_info: Optional dict from template_lookup
            taxonomy_actionable_type: Optional actionable type
    ...
    async def _process_single_partition(
        ...
        total_candidates = sum(len(r.categories) for r in map_results)
        ...
        if reduce_skipped:
            consolidated = ReducedCategoryList(
                categories=[
                    {
                        "category_label": c.category_label,
                        "description": c.description,
                        "merged_from": [c.category_label],
                    }
                    for c in map_results[0].categories
                ]
            )
        else:
        ...
        if verbose:
            reduce_info = ('reduce skipped'
                           if reduce_skipped
                           else f'reduced to {len(consolidated.categories)}')
        ...
        async def process_batch(batch_idx: int, labels: List[str]):
            ...
            try:
                result = await self._llm_call(
                    prompt, MapBatchCategories, self._max_tokens_map
                )
                ...
                print(f"    MAP '{partition_name}' batch "
                      ...
                      f"{type(e).__name__}: {e}")
                results[batch_idx] = MapBatchCategories(categories=[])
    ...
    async def _run_reduce_step(
        self,
        partition_name: str,
        map_results: List[MapBatchCategories],
        part_context: PartitionContext,
        prompt_context: PromptContext,
    ) -> ReducedCategoryList:
        """Consolidate categories from all batches into one unified list."""
        ...
        try:
            return await self._llm_call(
                prompt, ReducedCategoryList, self._max_tokens_reduce
            )
        except Exception as e:
    ...
    async def _run_mece_step(
        self,
        partition_name: str,
        categories: ReducedCategoryList,
        n_labels: int,
        part_context: PartitionContext,
        prompt_context: PromptContext,
    ) -> MECECategorySet:
        ...
        prompt = self._build_mece_prompt(
            categories, n_labels, part_context, prompt_context
        )
        ...
        try:
            ...
            return MECECategorySet(
                categories=[
                    MECECategory(
                        category_label=c.category_label,
                        inclusion_definition=c.description,
                        boundary_test="",
                        diagnostic_signals=[],
                        key_expressions=[],
                        tiebreaker_rules=[],
    ...
    def _build_reduce_prompt(
        ...
        for batch_idx, batch_result in enumerate(map_results):
            ...
            for cat in batch_result.categories:
                section_lines.append(
                    f"  - {cat.category_label}: {cat.description}"
                )
                if hasattr(cat, 'recognition_cue') and cat.recognition_cue:
                    section_lines.append(
                        f"    Recognition: A coder assigns here "
                        f"when it {cat.recognition_cue}"
                    )
            sections.append("\n".join(section_lines))

        batch_categories_list = "\n\n".join(sections)
        n_total_categories = sum(len(r.categories) for r in map_results)

        gi = part_context.grouping_instruction
        grouping_block = f"\n{gi}" if gi else ""

        return REDUCE_CATEGORIES_PROMPT.format(
            survey_question=prompt_context.survey_question,
            language=prompt_context.language,
            dataset_context_section=prompt_context.dataset_context_section,
            taxonomy_context=prompt_context.taxonomy_context,
    ...
    def _build_mece_prompt(
        ...
        for i, cat in enumerate(categories.categories, 1):
            cat_lines.append(
                ...
                f"   Merged from: {', '.join(cat.merged_from)}"
            )
        categories_list = "\n\n".join(cat_lines)
    ...
    def _build_taxonomy_context(
        ...
        When taxonomy_axis_info (from template_lookup.py) is available, builds
        an enriched block with allowed_concepts, excluded_concepts, and full
        ...
        if taxonomy_axis_info:
            noun_desc = taxonomy_axis_info.get(
                "noun_phrase_descriptor", taxonomy_axis
            )
            dimension_desc = taxonomy_axis_info.get(
                "dimension_description",
                taxonomy_description or "Not specified",
            )
            ...
            if allowed:
                lines.append(
                    f"Allowed concept types: {', '.join(allowed)}"
                )
            if excluded:
                lines.append(
                    f"Excluded concept types: {', '.join(excluded)}"
                )
            if taxonomy_actionable_type:
                lines.append(f"Actionable type: {taxonomy_actionable_type}")
            lines.append("")
            lines.append(
                "Categories MUST describe content within this dimension ONLY."
            )
            if excluded:
                lines.append(
                    f"Do NOT create categories about: {', '.join(excluded)}"
                )
