"""
Experimental IdeaExtractor for ideaExtractor_v3 - VERSION 3 (OPTIMAL STRATEGY)

This is the OPTIMAL rate limiting strategy implementation.

KEY CHANGES FROM V2:
1. Learned tiktoken→API token offset (accounts for ~150-200 token system overhead)
2. Real-time TPM tracking via sliding window (not just bucket state)
3. PID-style continuous throughput adjustment (gradual, not step changes)
4. Faster/more accurate bootstrap (5 probes instead of 3, better sampling)
5. Proactive adjustment triggers (before hitting limits, not after)

GOALS:
- Zero 429 errors
- Maximum throughput utilization (>90% of theoretical)
- Fast convergence to optimal rate

BASED ON: experiments/ideaExtractor_v2/ideaExtractor_experimentalv2.py
"""
...
# === EXPERIMENTAL PROMPTS (V3) =============================================================================
# Import from local v3 prompts (same as v2 prompts - only rate limiting strategy changed)
from experiments.ideaExtractor_v3.prompts import (
...
# === CONSTANTS (V3 OPTIMAL STRATEGY) ==================================================================================
# Token history windows
INPUT_HISTORY_MAXLEN = 10             # EMA input token history window
OUTPUT_HISTORY_MAXLEN = 15            # EMA output token history window
...
# V3 NEW: Tiktoken → API token offset learning
TIKTOKEN_API_OFFSET_DEFAULT = 300     # Default offset (instructor overhead ~300 tokens per request)
TIKTOKEN_OFFSET_HISTORY_MAXLEN = 30   # Samples to learn the offset
TIKTOKEN_OFFSET_MIN_SAMPLES = 5       # Min samples before using learned offset
...
# V3 IMPROVED: Faster intervals for quicker convergence
PROGRESS_REPORT_INTERVAL = 5          # Seconds between progress reports
DIAGNOSTIC_INTERVAL = 10              # V3: Reduced from 30s to 10s for faster adjustment
ADJUSTMENT_INTERVAL = 20              # V3: Check for adjustment every 20s

# V3 IMPROVED: Bootstrap settings
BOOTSTRAP_NUM_PROBES = 5              # V3: Increased from 3 to 5 for better accuracy
...
# V3 NEW: PID-style continuous adjustment (asymmetric gains)
PID_KP_UP = 0.4                       # Proportional gain when under-utilizing (speed up faster)
PID_KP_DOWN = 0.2                     # Proportional gain when over-utilizing (slow down gently)
PID_KI = 0.05                         # Integral gain (accumulated error correction)
PID_KD = 0.1                          # Derivative gain (dampen oscillations)
PID_MIN_ADJUSTMENT = 0.02             # Minimum adjustment to apply (2%)
PID_MAX_ADJUSTMENT = 0.15             # Maximum single adjustment (15%)

# V3 NEW: Real-time TPM tracking
TPM_SLIDING_WINDOW_SECONDS = 60       # Track TPM over last 60 seconds
TPM_SAMPLE_INTERVAL = 1.0             # Sample TPM every 1 second
TPM_TARGET_UTILIZATION = 0.85         # Target 85% TPM utilization (leaves 15% buffer)

# V3 CHANGED: Threshold-based adjustment (kept as fallback, but PID is primary)
THROUGHPUT_ADJUSTMENT_MIN_SAMPLES = 10  # V3: Reduced from 20 to 10 for faster learning
THROUGHPUT_ADJUSTMENT_THRESHOLD = 1.05  # V3: Reduced from 1.10 to 1.05 (more sensitive)

# Generic specifier settings (unchanged)
