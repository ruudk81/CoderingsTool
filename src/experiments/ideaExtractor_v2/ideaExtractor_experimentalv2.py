"""
Experimental IdeaExtractor for ideaExtractor_v2 - VERSION 2

This is a COPY of ideaExtractor_experimental.py that can be modified freely
...
FOCUS AREAS FOR V2:
- Improved ideas extraction via LLM prompting
- Improved taxonomy phrases extraction via LLM prompting

BASED ON: experiments/ideaExtractor_v2/ideaExtractor_experimental.py
"""
...
# === CONSTANTS ========================================================================================================
INPUT_HISTORY_MAXLEN = 10             # EMA input token history window (increased for stability)
OUTPUT_HISTORY_MAXLEN = 15            # EMA output token history window (increased for stability)
OUTPUT_RATIO_HISTORY_MAXLEN = 20      # Track output/input ratios for learning
DEFAULT_OUTPUT_RATIO = 0.25           # Initial fallback ratio (will be learned from data)
...
THROUGHPUT_ADJUSTMENT_MIN_SAMPLES = 20  # Min samples before adjusting throughput
THROUGHPUT_ADJUSTMENT_THRESHOLD = 1.10  # Adjust if actual > bootstrap × 1.10 (10% underestimation)
...
class TaxonomyEnrichedIdeaResponse(BaseModel):
    """Extended idea response with taxonomy phrase and parent category tag.

    V2: Ideas are standalone natural language units (no template prefix).
    Both idea and taxonomy_phrase end with ' - parent_category' tag.
    """
    ...
    idea: str = Field(description="Standalone natural language idea with parent category tag (format: 'idea text - parent_category')")
    taxonomy_phrase: str = Field(default="", description="5-12 word abstracted category phrase with tag (format: 'phrase - parent_category')")
    ...
    def normalize_taxonomy_phrase(cls, v: str) -> str:
        ...
        V2: Allows longer phrases (5-12 words) with parent category tag.
        Preserves ' - ' separator for parent category.
        """
        if not isinstance(v, str):
            return ""
        # Strip whitespace
        ...
        # Lowercase
        v = v.lower()
        # Remove trailing punctuation (keep internal hyphens and ' - ' separator)
        v = v.rstrip('.,;:!?')
        return v
...
class IdeaExtractor:
    ...
    def _calculate_avg_tokens(self) -> int:
        ...
        V2: Uses simplified prompt without template placeholders.
        """
    ...
    def _build_taxonomy_enriched_prompt(
        ...
        respondent_id: str,
        response: str,
        subject: str = None,  # Kept for API compatibility, not used in v2
        phrasing_template: str = None  # Kept for API compatibility, not used in v2
    ) -> str:
        """Build taxonomy-enriched prompt for idea extraction.

        V2: Simplified prompt that extracts standalone ideas with data-grounded
        parent category tags. No longer uses template prefixes.

        Args:
            respondent_id: Respondent identifier
            response: The response text to extract ideas from
            subject: (Deprecated in v2) Kept for API compatibility
            phrasing_template: (Deprecated in v2) Kept for API compatibility
    ...
    def estimate_tokens(self, prompt: str) -> int:
        """Estimate total tokens using adaptive strategy with learned output ratio.

        Features:
        - Dynamic safety margins: 30% early → 20% mid → 15% when learned
        - Learned output ratio instead of hardcoded 0.25
        - History-based blending for stable estimates
        """
        actual_input_tokens = len(self.encoding.encode(prompt))

        # Determine safety margin based on learning progress
        num_samples = len(self.estimation_errors)
        if num_samples < 5:
            safety_margin = 1.30  # 30% margin early (conservative)
        elif num_samples < 15:
            safety_margin = 1.20  # 20% margin mid (balanced)
        else:
            safety_margin = 1.15  # 15% margin when learned (efficient)
    ...
    async def process_task(self, task: Dict) -> models.IdeasExtractedModel:
        ...
        try:
            # V2: Build prompt directly without template prefix logic
            # Ideas are now standalone natural language units with data-grounded tags
            prompt = self._build_taxonomy_enriched_prompt(
                task['respondent_id'],
                task['response']
            )
            ...
            if self.prompt_printer and not self._captured_prompt:
                self.prompt_printer.capture_prompt(
                    ...
                    utility_name="IdeaExtractor",
                    prompt_content=prompt,
                    prompt_type="idea_extraction_v2",
                    metadata={
                        "model": self.model,
                        "var_lab": self.var_lab,
                        "language": self.language,
                ...
                async with self.rate_limiter:
                    ...
                    if ideas:
                        ...
                        return models.IdeasExtractedModel(
                            ...
                            template_prefix=""  # V2: No template prefix used
                        )
                    else:
    ...
    def build_extraction_metadata(self, filename: str = "", var_name: str = "") -> 'models.ExtractionMetadata':
        ...
        return models.ExtractionMetadata(
            ...
            # Template (V2: not used - ideas are standalone)
            template_prefix="",
    ...
    def _adjust_throughput_if_needed(self) -> bool:
        """Adjust throughput if actual token usage significantly exceeds bootstrap estimate.
        ...
        # Only adjust if we're significantly underestimating
        ...
        # Reinitialize token bucket with fresh state (clears any deficit from underestimation)
        ...
        if self.verbose_reporter.enabled:
            self.verbose_reporter.stat_line(f"Throughput adjusted: {old_tpm_throughput:.2f}/s → {new_arrival_rate:.2f}/s (actual tokens: {actual_avg:.0f})")
    ...
    async def process_all_tasks_async(self, tasks: List[Dict]) -> List[models.IdeasExtractedModel]:
        ...
        # === PHASE 5: Bootstrap probing with ACCURATE prompts ===
        if self.verbose_reporter.enabled:
            self.verbose_reporter.stat_line("\nV2 mode: Extracting standalone ideas with data-grounded parent categories")
            self.verbose_reporter.stat_line("Running bootstrap measurement (3 probe calls with real context)...")
            ...
            if self.verbose_reporter.enabled and now - last_diagnostics >= DIAGNOSTIC_INTERVAL:
                ...
                if token_stats['status'] == 'learning' and token_stats['samples'] >= 5:
                    self.verbose_reporter.stat_line(f"Token estimation: {token_stats['avg_estimation_error']:.0f} avg error, "
                                                  f"Input: {token_stats['avg_input_tokens']:.0f} avg ({token_stats['input_samples']}/{INPUT_HISTORY_MAXLEN}), "
                                                  f"Output: {token_stats['avg_output_tokens']:.0f} avg ({token_stats['output_samples']}/{OUTPUT_HISTORY_MAXLEN})")

                # Check if we need to adjust throughput based on actual vs estimated tokens
