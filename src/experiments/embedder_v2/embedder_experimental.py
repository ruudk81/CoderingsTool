"""
Experimental Embedder for embedder_v2

This is a COPY of src/utils/embedder.py that can be modified freely
for experimentation without affecting production code.

KEY CHANGES:
- Configurable embedding text format via experiment config
- Added embedding quality analysis methods
- Added similarity statistics computation

COPIED FROM: src/utils/embedder.py
"""
...
# === EXPERIMENTAL CONFIG ===========================================================================================
from experiments.embedder_v2.config import EmbedderExperimentConfig
...
class ExperimentalEmbedder:
    """Experimental embedder with configurable text format and quality analysis.

    Supports OpenAI and Gemini embedding providers with:
    - Configurable text format: "idea" (clean idea text) or "taxonomy_phrase"
    ...
    Args:
        experiment_config: EmbedderExperimentConfig with all experiment settings
    ...
    def __init__(
        self,
        experiment_config: EmbedderExperimentConfig = None,
        model_config: ModelConfig = None,
        client: Any = None,
        var_lab: str = None
    ):
        self.experiment_config = experiment_config or EmbedderExperimentConfig()
        self.model_config = model_config or ModelConfig()

        # Convert experiment config to embedding config
        self.config = self.experiment_config.to_embedding_config()

        self.provider = self.experiment_config.provider.lower()
        ...
        # Get embedding model
        if self.experiment_config.embedding_model:
            self.embedding_model = self.experiment_config.embedding_model
        else:
        ...
        # Store embedding text format from experiment config
        self.embedding_text_format = self.experiment_config.embedding_text_format

        # Analysis results
        self.analysis: Optional[EmbeddingAnalysis] = None

        # Extraction metadata (optional, for display/logging purposes only)
    ...
    def _get_text_for_embedding(self, idea) -> str:
        """Extract text for embedding based on experiment config.
    ...
    def analyze_embeddings(self, embeddings: List[np.ndarray]) -> EmbeddingAnalysis:
        ...
        if self.experiment_config.compute_similarity_stats and len(embeddings) <= 1000:
    ...
    async def _process_embeddings_with_id_tracking(self, data: List[models.EmbeddingsModel]) -> List[models.EmbeddingsModel]:
        ...
        if self.provider == "gemini":
            batch_size = self.experiment_config.gemini_batch_size
        elif self.provider == "openai":
            batch_size = self.experiment_config.openai_batch_size
        else:
            batch_size = 100
        ...
        if self.provider == "gemini":
            max_concurrent = self.experiment_config.gemini_max_concurrent
        elif self.provider == "openai":
            max_concurrent = self.experiment_config.openai_max_concurrent
        else:
            max_concurrent = 5
        ...
        if self.experiment_config.analyze_embeddings:
