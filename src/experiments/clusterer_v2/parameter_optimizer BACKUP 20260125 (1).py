"""
ClustererV2 Parameter Optimizer Module
...
class ParameterOptimizer:
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        if self._verbose:
            print(f"  Trial {trial.number}: nn={n_neighbors}, nc={n_components}, md={min_dist}, mcs={min_cluster_size}, ms={min_samples}")
        ...
        if self._verbose:
            print(f"    → COMPOSITE={composite_score:.4f} "
                  ...
                  f"pen_fuzzy_ct={score_breakdown['penalty_fuzzy_count']:.3f})")
            print(f"    → rel_validity={relative_validity:.4f}, k={n_clusters}, "
                  f"noise={noise_rate:.1%}, coh={coherence:.3f}")
            print(f"    → probs: mean={prob_metrics['mean_probability']:.2f}, "
                  f"low_ratio={prob_metrics['low_prob_ratio']:.1%} "
                  f"(excess={score_breakdown['excess_low_prob']:.1%})")
            print(f"    → fuzzy: {prob_metrics['n_fuzzy_clusters']}/{n_clusters} clusters "
                  f"({score_breakdown['fuzzy_cluster_fraction']:.1%}), "
                  f"points={prob_metrics['fuzzy_cluster_ratio']:.1%}")
            print(f"    → outliers: mean={outlier_metrics['mean_outlier_score']:.2f}, "
                  f"high_ratio={outlier_metrics['high_outlier_ratio']:.1%}")
    ...
    def optimize(self) -> OptunaResult:
        ...
        if self._verbose:
            ...
            print(f"  min_dist grid: {self._search_space['min_dist']}")
        ...
        self._study.optimize(self._objective, n_trials=None)  # Run all grid combinations
        ...
        if self._verbose:
            n_fuzzy = best.user_attrs.get('n_fuzzy_clusters', 0)
            k = best.user_attrs.get('n_clusters', 0)
            fuzzy_frac = n_fuzzy / k if k > 0 else 0.0
            print(f"\n[Optuna] Optimization complete")
            print(f"  Best: nn={n_neighbors}, nc={n_components}, md={min_dist}, mcs={min_cluster_size}, ms={min_samples}")
            print(f"  Composite score: {best.value:.4f}")
            print(f"    rel_validity={best.user_attrs.get('relative_validity', 'N/A'):.4f}, "
                  f"low_prob={best.user_attrs.get('low_prob_ratio', 'N/A'):.1%}, "
                  f"fuzzy={n_fuzzy}/{k}cl ({fuzzy_frac:.1%})")
        ...
        if not needs_research:
            if self._verbose:
                print(f"  Quality check PASSED: k={n_clusters} (expected≈{sqrt_n:.0f}), "
                      f"noise={noise_rate:.1%}, composite={composite_score:.3f}, rel_validity={relative_validity:.3f}")
            return result
    ...
    def _run_extended_search(self, initial_result: OptunaResult) -> OptunaResult:
        ...
        def extended_objective(trial: optuna.Trial) -> float:
            ...
            if self._verbose:
                ...
                print(f"  {improved} Trial {trial.number}: {method}, mcs={mcs}, ms={ms} → "
                      f"COMPOSITE={composite_score:.4f}, k={n_clusters}, noise={noise_rate:.1%}")
                print(f"      val={validity:.4f}, low_prob={prob_metrics['low_prob_ratio']:.1%}, "
                      f"fuzzy={prob_metrics['n_fuzzy_clusters']}/{n_clusters}cl "
                      f"({score_breakdown['fuzzy_cluster_fraction']:.1%}), "
                      f"pts={prob_metrics['fuzzy_cluster_ratio']:.1%}")

            return composite_score

        # Create and run extended search study
        ...
        if best_extended.value <= initial_result.best_value:
            if self._verbose:
                print(f"  No improvement found (best extended composite: {best_extended.value:.4f} <= initial: {initial_result.best_value:.4f})")
            return initial_result
