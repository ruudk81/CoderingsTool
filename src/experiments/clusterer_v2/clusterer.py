"""
Clusterer Main Module

Unified clustering utility with automatic algorithm selection,
...
from .parameter_optimizer import ParameterOptimizer, run_umap, n_neighbors_grid, mcs_grid_sqrt
...
class ClustererV2:
    ...
    Usage:
        ...
        config = ClustererV2Config(algorithm_mode="hdbscan")
        clusterer = ClustererV2(input_list, config=config)

        # With c-TF-IDF representation
        config = ClustererV2Config(generate_ctfidf=True)
        clusterer = ClustererV2(input_list, config=config)
    ...
    def _run_llm_labels(self):
        ...
        from .representation import extract_text_for_format
