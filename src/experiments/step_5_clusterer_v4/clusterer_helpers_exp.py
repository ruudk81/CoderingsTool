...
if USE_EXPERIMENTAL_PROMPTS:
    from .prompts_exp import CLUSTER_THEME_PROMPT, ClusterThemeDescription
else:
    from prompts_exp import CLUSTER_THEME_PROMPT, ClusterThemeDescription
...
def format_ontology_text(idea) -> str:
    """Format ontology fields into text: returns only the node value.

    Falls back to idea.idea when node is empty.
    """
    node = (getattr(idea, 'node', '') or '').strip()
    return node if node else getattr(idea, 'idea', '')


def format_full_ontology_text(idea) -> str:
    """Format full ontology chain: instance → node → semantic_category → root.

    Falls back to idea.idea when all ontology fields are empty.
    """
    parts = []
    for field_name in ('instance', 'node', 'semantic_category', 'root'):
...
def extract_embeddings(
    ...
    Returns:
        ...
        ontology_texts: List of ontology text strings (node value)
    ...
    return embeddings, idea_texts, ontology_texts, idea_indices, template_prefix, embedding_text_format
...
def preprocess_embeddings(
    ...
    """
    # Extract embeddings
    embeddings, idea_texts, ontology_texts, idea_indices, template_prefix, embedding_text_format = extract_embeddings(input_list, config)
    ...
    return embeddings_normalized, embeddings_processed, idea_texts, ontology_texts, idea_indices, pca_model, template_prefix, embedding_text_format


# Mapping: embedding_source → cached embedding field on idea (None = not cached)
EMBEDDING_FIELD_MAP = {
    "idea_embedding": "idea_embedding",
    "node_embedding": "node_embedding",
    "category_embedding": "category_embedding",
    "taxonomy_embedding": "taxonomy_embedding",
    # These have no cached embedding — embed on-the-fly from text field
    "category": None,
    "root": None,
    "instance": None,
}
...
def get_idea_field_text(idea, field: str) -> str:
    """Get text for a given field from an idea object.

    Args:
        idea: Idea object with ontology fields
        field: One of "idea", "instance", "node", "category", "root", "ontology"
               "ontology" returns the full chain "instance → node → semantic_category → root"

    Returns:
        Text string (may be empty if field is not populated)
    """
    if field == "ontology":
        return format_full_ontology_text(idea)
    if field == "idea":
        return idea.idea
    # Map legacy field name to new model field
    attr_name = "semantic_category" if field == "category" else field
    return (getattr(idea, attr_name, '') or '').strip()


def resolve_embeddings(
    input_list: List[models.EmbeddingsModel],
    idea_indices: List[Tuple[int, int]],
    source: str,
    verbose: bool = False,
) -> np.ndarray:
    """Get embeddings for the given source — cached if available, on-the-fly if not.

    Args:
        input_list: EmbeddingsModel list from step 4
        idea_indices: List of (response_idx, idea_idx) tuples
        source: Embedding source name (e.g. "idea_embedding", "taxonomy_embedding", "category", "root")
        verbose: Print progress info

    Returns:
        embeddings: Array of shape (n_ideas, embedding_dim)
    """
    cached_field = EMBEDDING_FIELD_MAP.get(source)

    if cached_field is not None:
        # Use pre-computed embedding from cache
        embeddings_list = []
        for resp_idx, idea_idx in idea_indices:
            idea = input_list[resp_idx].response_ideas[idea_idx]
            emb = getattr(idea, cached_field, None)
            if emb is not None:
                embeddings_list.append(emb)
            else:
                raise ValueError(
                    f"Idea {getattr(idea, 'idea_id', '?')} has no {cached_field}. "
                    f"Re-run step 4 with a compatible embedding_text_format."
                )
        if verbose:
            print(f"  Using cached embedding field: {cached_field}")
        return np.vstack(embeddings_list)

    # No cached embedding — embed the text field on-the-fly
    import asyncio
    from utils.llm import create_embedding_client
    from config import get_embedding_model_for_api

    # Collect text per idea
    texts = []
    for resp_idx, idea_idx in idea_indices:
        idea = input_list[resp_idx].response_ideas[idea_idx]
        texts.append(get_idea_field_text(idea, source))

    # Embed unique texts only (many ideas may share the same category/root)
    unique_texts = list(dict.fromkeys(texts))
    n_empty = sum(1 for t in unique_texts if not t)
    if n_empty:
        print(f"  WARNING: {n_empty} unique '{source}' values are empty")

    if verbose:
        print(f"  On-the-fly embedding: {len(unique_texts)} unique '{source}' texts "
              f"for {len(texts)} ideas")

    async def _embed_async(to_embed: List[str]) -> np.ndarray:
        client = create_embedding_client(async_mode=True)
        model = get_embedding_model_for_api()
        response = await client.embeddings.create(input=to_embed, model=model)
        return np.array([item.embedding for item in response.data], dtype=np.float32)

    unique_embeddings = asyncio.run(_embed_async(unique_texts))

    # Map each idea to its text's embedding
    text_to_idx = {t: i for i, t in enumerate(unique_texts)}
    embeddings = np.array([unique_embeddings[text_to_idx[t]] for t in texts])
...
def resolve_embeddings_unique(
    input_list: List[models.EmbeddingsModel],
    idea_indices: List[Tuple[int, int]],
    source: str,
    verbose: bool = False,
) -> Tuple[np.ndarray, List[str], List[int]]:
    """Embed unique field values only — for clustering unique points (V2 pattern).

    Instead of returning one embedding per idea (with duplicates for shared
    categories), returns only the unique embeddings and a mapping from each
    idea to its unique index. This lets HDBSCAN cluster N_unique points
    instead of N_ideas points with degenerate duplicates.

    Args:
        input_list: EmbeddingsModel list from step 4
        idea_indices: List of (response_idx, idea_idx) tuples
        source: Field name to embed (e.g. "category", "root")
        verbose: Print progress info

    Returns:
        unique_embeddings: (n_unique, dim) array
        unique_texts: List of unique text strings (length n_unique)
        idea_to_unique_idx: List mapping each idea flat index to its unique index
    """
    import asyncio
    from utils.llm import create_embedding_client
    from config import get_embedding_model_for_api
    ...
    unique_texts = list(dict.fromkeys(texts))
    text_to_idx = {t: i for i, t in enumerate(unique_texts)}
    idea_to_unique_idx = [text_to_idx[t] for t in texts]
    ...
    return unique_embeddings, unique_texts, idea_to_unique_idx
...
def run_umap(
    ...
    # Clamp n_components and n_neighbors to avoid scipy eigsh failure (k >= N)
    n_components = min(n_components, N - 1) if N > 1 else 1
    n_neighbors = min(n_neighbors, N - 1) if N > 1 else 1

    # Use random init when N is too small for spectral layout
    init = "random" if N <= n_components + 1 else "spectral"
...
class ParameterOptimizer:
    ...
    def optimize(self) -> OptunaResult:
        ...
        if candidates and self._verbose:
            ...
            # Check if any trials completed before attempting extended search
            completed = [t for t in self._study.trials if t.state == optuna.trial.TrialState.COMPLETE]
            if not completed:
                if self._verbose:
                    print(f"\n[Extended Search] Skipped — no completed trials to seed from")
            else:
                if self._verbose:
        ...
        if not candidates:
            # Graceful fallback: assign all points to a single cluster
            if self._verbose:
                print(f"\n[Fallback] No completed trials — assigning all {self._N} points to cluster 0")
            n_comp = min(2, self._N - 1) if self._N > 2 else 1
            n_neigh = min(5, self._N - 1)
            fallback_umap = run_umap(self._embeddings, n_neigh, n_comp)
            return OptunaResult(
                best_params={'n_neighbors': n_neigh, 'n_components': n_comp,
                             'min_dist': 0.1, 'min_cluster_size': 2},
                best_value=0.0,
                best_labels=np.zeros(self._N, dtype=int),
                best_model=None,
                n_trials_completed=0,
                n_trials_pruned=len([t for t in self._study.trials if t.state == optuna.trial.TrialState.PRUNED]),
                study=self._study,
                umap_embeddings=fallback_umap,
                search_space=self._search_space,
                persistence_metrics={},
            )
...
def extract_text_for_format(
    ...
    """Extract text for the given canonical format name.

    Called with canonical format names ('ontology', 'idea')
    derived from the clusterer's embedding_source config.
    """
        ...
        for cluster_id, texts in cluster_texts.items():
            ...
            for i, text in enumerate(texts):
                ...
                cleaned_texts.append(
                    extract_text_for_format(text, embedding_text_format, ontology_text=ontology)
                )
...
# =============================================================================
# SECTION 7: THEME GENERATION (V2)
# =============================================================================

@dataclass
class ClusterTheme:
    """Container for cluster theme information (V2: replaces ClusterLabel)."""
    cluster_id: int
    theme: str
...
class ThemeGenerator:
    """
    LLM-based cluster theme generator (V2: replaces LabelGenerator).

    Generates per-cluster topics with inclusion definitions instead of
    checkbox-style code labels.

    Usage:
        generator = ThemeGenerator(config)
        themes = generator.generate_all_themes(cluster_texts, cluster_keywords)
    """
    ...
    def __init__(self, config: ClustererConfig, prompt_template=None, response_model=None):
        ...
        Args:
            config: ClustererConfig
            prompt_template: Override prompt template (default: CLUSTER_THEME_PROMPT)
            response_model: Override Pydantic response model (default: ClusterThemeDescription)
        """
        self.config = config
        self._model = config.llm_labels_model
        self._max_ideas = config.llm_max_ideas_per_cluster
        self._prompt_template = prompt_template or CLUSTER_THEME_PROMPT
        self._response_model = response_model or ClusterThemeDescription
    ...
    def _build_cluster_profile_section(self, distributions: Optional[Dict[str, Dict[str, float]]]) -> str:
        """Build cluster profile section (placeholder — sentiment/sense removed)."""
        return ""
    ...
    def generate_theme(
        ...
        """Generate LLM-based theme for a single cluster."""
...
"""
        ...
        try:
            ...
            theme = ClusterTheme(
                ...
                inclusion_definition=description.inclusion_definition,
                key_concepts=description.key_concepts,
                n_ideas=len(ideas)
            )
            return (theme, prompt) if return_prompt else theme
            ...
            theme = ClusterTheme(
                ...
                inclusion_definition="Theme generation failed",
                key_concepts=[],
                n_ideas=len(ideas)
            )
            return (theme, prompt) if return_prompt else theme
    ...
    def generate_all_themes(
        ...
        """Generate LLM-based themes for all clusters (Phase B)."""
        ...
        if verbose:
            print(f"\n[Theme Generation (Phase B)]")
            print(f"  Model: {self._model}")
            print(f"  Clusters to process: {len(cluster_texts)}")
            if taxonomy_axis:
            ...
            if cluster_distributions:
                print(f"  Including cluster profiles")
        ...
        for i, (cluster_id, ideas) in enumerate(sorted(cluster_texts.items())):
            ...
            if verbose:
                ...
                print(f"  Generating theme for cluster {cluster_id} ({len(ideas)} ideas{sample_info})...", end=" ")

            result = self.generate_theme(
                cluster_id=cluster_id,
                ideas=ideas,
            ...
            if is_first:
                theme, sample_prompt = result
            else:
                theme = result
            themes[cluster_id] = theme

            if verbose:
                theme_display = theme.theme[:50] + "..." if len(theme.theme) > 50 else theme.theme
                print(f"'{theme_display}'")

        if verbose and sample_prompt:
            print(f"\n  [Sample Theme Prompt (cluster 0)]")
            print("  " + "-" * 70)
