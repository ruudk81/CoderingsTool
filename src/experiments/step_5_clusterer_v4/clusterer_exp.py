"""
Clusterer V4 — Clustering Infrastructure (Phases 1-5 only)

EXPERIMENTAL VERSION (step_5_clusterer_v4)
Changes here do NOT affect the production pipeline.

Based on V3 clusterer with Phase 7 (Map-Reduce MECE) removed.
In V4, topic extraction is orchestrated externally by run_experiment.py
via ObjectDiscoverer + ObjectMapper + ObjectAwareMapReduceMECE.

This Clusterer handles only:
- Phase 1: Preprocessing (extract, normalize, PCA)
- Phase 2: Algorithm selection (DVC + knee)
- Phase 3: Clustering (HDBSCAN / Agglomerative / K-means)
- Phase 4: Post-processing (merge, noise reduction)
...
Usage:
    ...
    config = ClustererConfig(embedding_source="category")
...
class Clusterer:
    """
    Clustering module (phases 1-5 only) for V4 object-aware pipeline.
    ...
    4. On-the-fly unique-point clustering for category/root levels
    ...
    def __init__(
        ...
        # expand labels back to all ideas before downstream stages)
    ...
    def run(self) -> 'Clusterer':
        ...
        if self._verbose:
            print("=" * 70)
            print("Clustering Pipeline V4 (Phases 1-5)")
            print("=" * 70)
        ...
        # V4: Topic extraction handled externally by run_experiment.py
    ...
    def _run_preprocessing(self):
        ...
        # then expand labels back to all ideas before downstream stages
        if is_on_the_fly:
    ...
    def _expand_labels_to_ideas(self):
        ...
        unique embedding point. Restores original idea state for downstream stages.
        """
    ...
    def get_labels(self) -> np.ndarray:
        """Get cluster labels array."""
        if self._labels is None:
            raise RuntimeError("Must call run() before get_labels()")
        return self._labels

    def get_idea_indices(self) -> List[Tuple[int, int]]:
        """Get (resp_idx, idea_idx) tuples for all ideas."""
        if self._idea_indices is None:
            raise RuntimeError("Must call run() before get_idea_indices()")
        return self._idea_indices

    def get_idea_texts(self) -> List[str]:
        """Get idea text list (aligned with labels)."""
        if self._idea_texts is None:
            raise RuntimeError("Must call run() before get_idea_texts()")
        return self._idea_texts

    def get_input_list(self) -> List[models.EmbeddingsModel]:
    ...
    def print_all_clusters(self, n_samples: int = 5):
        """Print all clusters with sample ideas and probability statistics."""
        if self._labels is None:
        ...
        probs = self._expanded_probabilities
