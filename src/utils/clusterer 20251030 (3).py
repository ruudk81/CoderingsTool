...
class Clusterer:
    ...
    def _assess_noise_quality(self, U: np.ndarray, labels: np.ndarray, centroids: Dict[int, np.ndarray]) -> Dict[str, float]:
        """
        ...
        Returns:
            Dictionary with:
                ...
                - soft_noise_rate: fraction assignable (similarity >= threshold)
                - hard_noise_rate: fraction truly problematic (similarity < threshold)
                - mean_noise_similarity: average max similarity of noise points to clusters
        """
        noise_mask = labels == -1
        n_total = len(labels)
        ...
        # Compute max similarity of each noise point to any cluster centroid
        # Since U is already L2-normalized in the pipeline, dot product = cosine similarity
        similarities = noise_points @ centroid_matrix.T  # (n_noise, n_clusters)
        max_similarities = similarities.max(axis=1)  # (n_noise,)

        # Classify noise by assignability
        threshold = self.clustering_config.noise_assignability_threshold
        assignable = max_similarities >= threshold
        ...
        return {
            ...
            'mean_noise_similarity': float(max_similarities.mean())
        }
    ...
    def _evaluate_hdbscan(self, U: np.ndarray, ms: int, mcs: int) -> Dict[str, Any]:
        ...
        # Compute centroids for noise assessment
        centroids = {}
        if n_clusters > 0:
            by_cluster = defaultdict(list)
            for vec, label in zip(U, labels):
                if label >= 0:
                    by_cluster[int(label)].append(vec)
            for cluster_id, vectors in by_cluster.items():
                vectors_array = np.vstack(vectors)
        ...
        # Assess noise quality (soft vs hard)
        noise_breakdown = self._assess_noise_quality(U, labels, centroids)

        # Calculate ALL metrics including expensive ones
        M = self._calculate_metrics(U, labels, db)

        # Add noise breakdown to metrics
        M.update({
            'soft_noise_rate': noise_breakdown['soft_noise_rate'],
            'hard_noise_rate': noise_breakdown['hard_noise_rate'],
            'mean_noise_similarity': noise_breakdown['mean_noise_similarity']
        })
    ...
    def _auto_hdbscan_grid(self, U: np.ndarray):
        ...
        hard_noise0 = best["metrics"].get("hard_noise_rate", np.nan)
        ...
        while rounds > 0 and changed:
            ...
            ms_new, mcs_new, note, changed = self._apply_threshold_rule(
                ms_best, mcs_best,
                dbcv=dbcv0, hard_noise=hard_noise0,
            ...
            hard_noise0 = best2["metrics"].get("hard_noise_rate", np.nan)
