...
class IdeaExtractor:
    ...
    async def _process_all_responses(self):
        ...
        batches = self._batch()
        # Calculate total concurrent tasks for reporting
        self.verbose_reporter.stat_line(f"Processing in {len(batches)} batches .... ")
    ...
    def extract(self) -> List[models.IdeasExtractedModel]:
        ...
        self.verbose_reporter.stat_line("  • Model: {self.config.model}")
        self.verbose_reporter.stat_line("  • Temperature: {self.config.temperature}")
        self.verbose_reporter.empty_line()
        ...
        if response_examples:
            ...
            for example in response_examples:
                ...
                for idea in example['ideas']:
                    print(f'    → "{idea}"')
                if example != response_examples[-1]:  # Add blank line between examples except after last
                    print()
