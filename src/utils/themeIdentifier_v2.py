...
# ============================================================================
# HIGH-PERFORMANCE THEME IDENTIFIER V2
# ============================================================================
...
class ThemeIdentifierV2:
    ...
    def __init__(self, 
        ...
        # Configuration - V2 Performance Optimizations
        ...
        # V2 Real-time Theme Memory (following codeGenerator pattern)
    ...
    # V2 PERFORMANCE OPTIMIZATIONS - HIERARCHICAL CONCURRENCY
    ...
    async def _process_noise_codes_concurrently(self, noise_codes: List[CodeEmbedding]) -> List[Dict[str, Any]]:
        """Process noise codes with optimized concurrent processing - V2 Performance Enhancement"""
        ...
        # V2 OPTIMIZATION: Remove artificial delay and use larger batches
        ...
        for i in range(0, len(noise_codes), batch_size):
            ...
            # V2 OPTIMIZATION: NO MORE ARTIFICIAL DELAYS
    ...
    async def identify_themes_by_clustering(self) -> Dict[str, Any]:
        """
        V2 Main method: High-performance theme identification using hierarchical concurrency
        """
        
        self.verbose_reporter.section_header("CLUSTERING-BASED THEME IDENTIFICATION V2 (OPTIMIZED)")
        ...
        if not self.codebook:
            ...
            return {
                'codebook': [],
                'themes': [],
                'methodology': 'Clustering-based Braun & Clarke (2006) - V2 Optimized'
            }
        ...
        # Step 5: Process clusters with V2 hierarchical concurrency
        identified_themes = []
        ...
        # V2 OPTIMIZATION: Process regular clusters with hierarchical concurrency
        if cluster_ids:
            ...
            if self.prompt_printer and batches and batches[0]:
                ...
                self.prompt_printer.capture_prompt(
                    step_name="clustering_based_theme_identification_v2",
                    utility_name="ThemeIdentifierV2",
                    prompt_content=sample_prompt,
                    prompt_type="Clustering-based Theme Decision V2",
                    metadata={
                        "model": self.model_config.get_model_for_stage("theme_synthesis"),
        ...
        if -1 in clusters:
            ...
            self.verbose_reporter.stat_line(f"\nProcessing {len(noise_codes)} noise codes with V2 optimizations")
            ...
            if self.prompt_printer and noise_codes:
                ...
                self.prompt_printer.capture_prompt(
                    step_name="individual_noise_code_assignment_v2",
                    utility_name="ThemeIdentifierV2",
                    prompt_content=sample_prompt,
                    prompt_type="Individual Noise Code Assignment V2",
                    metadata={
                        "model": self.model_config.get_model_for_stage("theme_synthesis"),
                        "var_lab": self.var_lab,
                        "language": DEFAULT_LANGUAGE,
    ...
    async def _finalize_theme_identification(
        ...
        # Report results
        self.verbose_reporter.summary("V2 THEME IDENTIFICATION COMPLETE", {
            ...
            "Methodology": "V2 - Clustering-based Braun & Clarke (2006) with concurrency optimizations"
        })
    ...
    def _build_final_codebook_structure(self, themes: List[ThemeStructure]) -> Dict[str, Any]:
        ...
        return {
            ...
            'methodology': 'V2 - Clustering-based Braun & Clarke (2006) with hierarchical concurrency',
