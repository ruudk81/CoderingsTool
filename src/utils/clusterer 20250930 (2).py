...
class Clusterer:
    ...
    def _dbcv_score(self, X: np.ndarray, labels: np.ndarray) -> float:
        ...
        X_clean = X_clean.astype(np.float32, copy=False)
    ...
    def gini_from_sizes(self,sizes):
    ...
    def _auto_hdbscan_grid(self,  U: np.ndarray,  embeddings: np.ndarray, mcs_grid: Optional[Iterable[int]] = None) -> Tuple[HDBSCAN, np.ndarray, ClusterSummary]:
        ...
        # stab_n= self._scale_metric01(stab_t)

        geometry     = 0.5*sil + 0.5 *db
        semantics    = 0.5 *coh + 0.5* exc
        compact_raw  = (geometry + semantics) / 2.0      
        #compact_n    = self._scale_metric01(compact_raw) 
        balance      = stab
        
        base_raw     = compact_raw  
        ...
        penalties   = (noise + k_n)/2
        #pen_n      = self._scale_metric01(penalties) 
        
        final_score = 1 + base_raw - penalties
