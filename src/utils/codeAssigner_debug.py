"""
Debug version of codeAssigner with comprehensive concurrency tracking.
This is a temporary version to identify the bottleneck issue.
"""
...
class CodeAssignerDebug:
    ...
    async def _get_code_embeddings(self):
        ...
        if self._code_embeddings is None:
            ...
            async with self.debugger.track_api_call("code_embeddings", "embedder_batch"):
    ...
    def _create_batches(self, all_ideas: List[tuple]) -> List[List[tuple]]:
        ...
        # Log batch creation details
        print(f"\n[DEBUG] Batch creation details:")
        print(f"  - Total ideas: {len(all_ideas)}")
        print(f"  - Number of batches created: {len(batches)}")
        print(f"  - Average batch size: {len(all_ideas) / len(batches):.1f}" if batches else "0")
        print(f"  - Token budget per batch: {token_budget}")
        print(f"  - Adaptive max batch size: {adaptive_max_batch}")
    ...
    async def assign_codes(self) -> List[models.CodeAssignedModel]:
        ...
        self.verbose_reporter.section_header("CODE ASSIGNMENT PROCESSING (DEBUG)")
        ...
        # Generate and display debugging report
        print("\n" + "="*80)
        ...
        for key, value in report['performance_metrics'].items():
            print(f"  - {key}: {value:.3f} seconds" if 'duration' in key else f"  - {key}: {value}")
        ...
        if report['potential_bottlenecks']:
            print(f"\nPOTENTIAL BOTTLENECKS DETECTED: {len(report['potential_bottlenecks'])}")
        ...
        timeline_path = f"/workspaces/CoderingsTool/codeAssigner_timeline_{int(time.time())}.json"
