...
CLUSTER_METRIC = ClusteringConfig.CLUSTER_METRIC
DBCV_D = ClusteringConfig.DBCV_D  # safe for DBCV (avoid overflow)
...
class Clusterer:
    ...
    def _pca_reduce(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        pca_embeddings = pca.fit_transform(embeddings).astype(np.float64, copy=False)
    ...
    def _dbcv_score(self, X: np.ndarray, labels: np.ndarray) -> float:
        ...
        return float(validity_index(X_clean, y_clean, metric=CLUSTER_METRIC, d=DBCV_D))
    ...
    def _silhouette_score(self, X: np.ndarray, labels: np.ndarray, sample_size: Optional[int] = None) -> float:
        ...
        return float(silhouette_score(X_clean, y_clean, metric=CLUSTER_METRIC, sample_size=sample_size, random_state=self.clustering_config.pca_random_state))
    ...
    def _topk_intra_centroid_angular(self, X, labels, topk, trim_q, robust):
        ...
        mask = labels >= 0
        if not np.any(mask):
            return {'top_ids': [], 'per_cluster': {}, 'weighted_mean': np.nan}
        ...
        for cid in top_ids:
            ...
            Xi = Xn[idx]
            if Xi.shape[0] < 2:
                per_cluster[int(cid)] = {'size': int(Xi.shape[0]), 'mean': np.nan,'hmean': np.nan, 'q25': np.nan}
                continue
            centroid = Xi.mean(axis=0, keepdims=True)
            sims = cosine_similarity(Xi, centroid).ravel()  # [-1,1], usually [0,1]
            if trim_q > 0.0:
                lo = np.quantile(sims, trim_q)
                hi = np.quantile(sims, 1 - trim_q)
                sims = sims[(sims >= lo) & (sims <= hi)]
            if sims.size == 0:
                m = h = q = np.nan
            else:
                m = float(np.mean(sims))
                if robust:
                    h = float(len(sims) / np.sum(1.0 / np.clip(sims, 1e-6, 1.0)))
                    q = float(np.quantile(sims, 0.25))
                else:
                    h = q = np.nan
            per_cluster[int(cid)] = {'size': int(Xi.shape[0]), 'mean': m, 'hmean': h, 'q25': q}
            sizes.append(Xi.shape[0]); means.append(m)
    
        sizes = np.array(sizes, float)
        weights = sizes / sizes.sum() if sizes.size else np.array([np.nan])
        weighted_mean = float(np.nansum(weights * np.array(means, float))) if sizes.size else np.nan
    ...
    def _calculate_metrics(self, U: np.ndarray, labels: np.ndarray, hdbscan_model: HDBSCAN) -> Dict[str, float]:
        ...
        # Global metrics
        self.enable_dbcv = False
        if self.enable_dbcv:
            dbcv = self._dbcv_score(U, labels)
        else: 
            dbcv = "N/A"
        sil = self._silhouette_score(U, labels)
        meanp = self._mean_probability(hdbscan_model, labels)
        ...
        if X.shape[0] and n_clusters >= 2:
            sil_samples = silhouette_samples(X, y, metric=CLUSTER_METRIC)
            # Fraction of low-sil points
            frac_low_points = float((sil_samples < 0.0).mean())
            ...
            frac_low_clusters = float((cluster_means < 0.20).mean())
        ...
        return {
            ...
            "frac_low_points": frac_low_points,
            "frac_low_clusters": frac_low_clusters
        }
    ...
    def _evaluate_hdbscan(self, U: np.ndarray, mcs: int, embeddings: np.ndarray) -> Dict[str, Any]:
        ...
        M = self._calculate_metrics_with_sampling(U, labels, db)
    ...
    def _calculate_metrics_with_sampling(self, U: np.ndarray, labels: np.ndarray, hdbscan_model: HDBSCAN) -> Dict[str, float]:
        """Enhanced _calculate_metrics with intelligent sampling"""
        ...
        sil = self._silhouette_score(U, labels)
        meanp = self._mean_probability(hdbscan_model, labels)
        cdist, cdist5 = self._centroid_distance_topk(U, labels, topk_clusters=5)
        wm_ang5 = self._topk_intra_centroid_angular(U, labels, topk=5, trim_q=0.0, robust=False)
    ...
    def _auto_hdbscan_grid(self,  U: np.ndarray,  embeddings: np.ndarray, mcs_grid: Optional[Iterable[int]] = None) -> Tuple[HDBSCAN, np.ndarray, ClusterSummary]:
        ...
        def _zscore01(x, winsor=2.0):
            """Z-score then map to ~[0,1]; winsorize to limit outliers. Safe for small n."""
            ...
            return (z + winsor) / (2*winsor)   # [-w,+w] -> [0,1]
        ...
        # # --- buckets
        # geometry  = 0.5*sil + 0.5* (1-db)
        # semantics = 0.5*coh + exc
        # balance   = 1-gini
        
        #2) monotone transforms (sharpen + invert DB, invert Gini)
        ...
        #3) scale each to ~[0,1] using z-score (or swap in minmax/rank if you prefer)
        sil_n = _zscore01(sil_t)
        db_n  = _zscore01(dbb_t)
        coh_n = _zscore01(coh_t)
        exc_n = _zscore01(exc_t)
        g_n   = _zscore01(g_t)     
        ...
        # 4) penalties
        wm_ang5 = np.array([r["metrics"].get("wm_ang5", np.nan) for r in results], float)
        ...
        mcs     = np.array([r["mcs"] for r in results], dtype=float)
        ratio   = np.clip((k * mcs) / max(n, 1), 0.0, 1.0)
        alpha_k = 2.0   # strength of k⊗mcs penalty
        sizeC    = np.minimum(ratio * alpha_k, 1.0)
        
        wm_ang5_n = _zscore01(wm_ang5)  
        sizeC_n    = _zscore01(sizeC)
        
        beta_ang5  = 1.0
        beta_k     = 1.0   # strength of k⊗mcs penalty
        beta_noise = 0   # weight for noise penalty
        beta_badC  = 0   # weight for "bad clusters" fraction
        
        pen_ang5   = beta_ang5 * np.nan_to_num(wm_ang5_n, nan=0.0)
        pen_sizeC  = beta_k * np.nan_to_num(sizeC_n, nan=0.0)
        pen_noise  = beta_noise * np.nan_to_num(noise, nan=0.0)
        pen_badC   = beta_badC * np.nan_to_num(badC, nan=0.0)

        penalties = pen_ang5 + pen_sizeC + pen_noise + pen_badC
      
        # 5) composite score
        w_geom, w_sem, w_bal, w_pen = 1/3, 1/3, 1/3, 1.0
        base_score = w_geom*geometry + w_sem*semantics + w_bal*balance
        final_score = base_score - w_pen*penalties
        ...
        for i, r in enumerate(results):
            ...
            r["penalty_ang5"]    = float(pen_ang5[i])
            r["penalty_k_mcs"]   = float(pen_sizeC[i])
            r["penalty_noise"]   = float(pen_noise[i])
            r["penalty_badC"]    = float(pen_badC[i])
            r["penalties_sum"]   = float(penalties[i])
        ...
        for i, result in enumerate(results):
            ...
            self.verbose_reporter.stat_line(
                ...
                f"pen={result['penalties_sum']:.3f} | "
                f"ang5={result.get('penalty_ang5', float('nan')):.3f} | "
                f"k_mcs={result['penalty_k_mcs']:.3f} | "
                f"noise={result.get('penalty_noise', float('nan')):.3f} | "
                f"badC={result['penalty_badC']:.3f}")
        ...
        for i, result in enumerate(results):
            ...
            self.verbose_reporter.stat_line(
                f"mcs={result['mcs']:>4}: | " 
                f"meanp={metrics.get('meanp', float('nan')):.3f} | "
                f"{dbcv_str}"
                f"Sil={metrics.get('sil', float('nan')):.3f} | "
                f"DB={metrics.get('DB', float('nan')):.3f} | "
                f"cdist={metrics.get('cdist', float('nan')):.3f} | "
                f"cdist5={metrics.get('cdist5', float('nan')):.3f} | "
