...
class ClusterGenerator:
    ...
    def _cosine_similarity_rescue(self, noise_indices: np.ndarray, embeddings: np.ndarray, labels: np.ndarray) -> int:
        ...
        for noise_idx in noise_indices:
            ...
            if best_similarity >= threshold:
                ...
                self.output_list[noise_idx].initial_idea_cluster = best_cluster_id
            ...
            if self.config.noise_rescue.use_cosine_rescue:
                ...
                for i, noise_idx in enumerate(noise_indices):
                    if strengths[i] > threshold:
                        self.output_list[noise_idx].initial_idea_cluster = rescued_clusters[i]
                        rescued_count += 1
                ...
                if rescued_count > 0:
                    ...
                    for i, noise_idx in enumerate(noise_indices):
                        if strengths[i] > threshold and example_count < 3:
                            item_text = self.output_list[noise_idx].idea or "N/A"
                            cluster_id = self.output_list[noise_idx].initial_idea_cluster
    ...
    def filter_and_remap_clusters(self) -> None:
        ...
        items = [item.idea for item in self.output_list]
        ...
        for i, item in enumerate(self.output_list):
            ...
            item.initial_idea_cluster = new_cluster
