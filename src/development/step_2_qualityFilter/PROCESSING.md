# Step 2: Quality Filter — Processing Reference

Reference for debugging and improvement. Source of truth: the code in `qualityFilter_exp.py`.
...
Nano: raw API + regex/tag parsing (Pattern A). Mini/default: instructor + Pydantic validation (Pattern B). No mixing — the tier determines the path at init.
...
Step 2 evaluates each survey response individually and flags low-quality items (don't know, empty, gibberish, etc.). One API call per response. Single-phase worker/queue dispatch.

- **Input:** `List[PreprocessedModel]` (from step 1)
- **Output:** `List[QualityFilteredModel]` with `quality_filter_code` set
- **Model:** Configured via `get_step_model("quality_filter")` — typically nano, supports mini/default
...
Minimal API call (`"Hi"`) to read `x-ratelimit-limit-tokens` and `x-ratelimit-limit-requests` from response headers. Falls back to `FALLBACK_TPM` / `FALLBACK_RPM` if headers are missing.

Provider-aware: uses `responses.create()` for OpenAI, `chat.completions.create()` for Azure.

### Phase 2: Initialize rate limiters

`_initialize_rate_limiters()` sets up all four control layers:
...
Workers are spawned upfront: `min(target_semaphore, num_tasks)`, floor of 5.

### Phase 3: Warm-up calibration

Triggers after `WARM_UP_WINDOW_SECONDS` (10s) AND `WARM_UP_MIN_COMPLETIONS` (3) completions.

- Measures actual avg tokens and median latency from completed tasks
...
- Logs success/failure/timeout counts
- Persists measured P50/P95 latency and avg tokens to `data/model_perf_stats.json` via EMA update (requires >= 5 latency samples)
...
- Merges recovered results back, reports recovery count and permanently failed IDs
...
`_apply_pid_adjustment()` runs every 15s. Continuously tunes the AsyncLimiter arrival rate based on real-time TPM utilization.

- `RealTimeTPMTracker` — 60s sliding window of actual token consumption, fed after every API call
- `PIDThroughputController` — asymmetric gains (kp_up=0.4 aggressive, kp_down=0.2 gentle), targets 80% TPM utilization
- Adjustment clamped to +/-15% per step, ignored if <2% change
- PID state reset after warm-up calibration to avoid carrying stale integral/derivative
...
| Prompt | `GRADER_INSTRUCTIONS_NANO` with `<scratchpad>`, `<category>` tags | `GRADER_INSTRUCTIONS_STRUCTURED` with JSON schema hint |
...
Both share: same prompt body (categories, decision rules), same rate limiting, same result model.
...
| Aspect | OpenAI | Azure |
|---|---|---|
| Probe call | `responses.with_raw_response.create()` | `chat.completions.with_raw_response.create()` |
| Model param | `self.model` | `AZURE_OPENAI_DEPLOYMENT_NAME` |

Main `process_task()` uses `llm.py` for provider abstraction.
...
Code: `TIMEOUT_FLOOR_SECONDS = 60.0`. Strategy doc: 20s for single-processing steps. On first-ever run (no stored stats), this is 3x the doc's recommendation.
...
|---|---|---|
| `COLD_START_CAP` | 50 | `config_exp.py` |
| `WARM_UP_WINDOW_SECONDS` | 10.0 | `config_exp.py` |
| `WARM_UP_MIN_COMPLETIONS` | 3 | `config_exp.py` |
| `RAMP_INCREASE_FACTOR` | 1.10 (+10%) | `config_exp.py` |
| `RAMP_DECREASE_FACTOR` | 0.80 (-20%) | `config_exp.py` |
| `SIGNAL_GREEN_THRESHOLD` | 0.80 | `config_exp.py` |
| `SIGNAL_YELLOW_THRESHOLD` | 0.90 | `config_exp.py` |
| `TIMEOUT_FLOOR_SECONDS` | 60.0 | `qualityFilter_exp.py` hardcoded |
| `COLD_START_P95_MULTIPLIER` | 2.0 | `modelPerfStats.py` |
| Post-warm-up jump cap | 100 | `qualityFilter_exp.py` hardcoded |
| Retry worker fraction | 10% (min 5) | `qualityFilter_exp.py` hardcoded |
