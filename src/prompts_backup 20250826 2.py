...
Your task is to identify the dominant shared idea(s) in a cluster of responses and express them as atomic, grounded theme statements.
...
- The ideas are **clearly distinct** (i.e., cannot be merged without compounding), AND
- Each idea is **supported by repeated patterns across multiple respondents**.
...
QUALITY CRITERIA
────────────────────────────────────────
- **ATOMIC**: Each theme_statement must express **only one idea** — no combinations of subthemes, sub-ideas, or sub-concepts.
...
- **CONCISE**: Each theme_statement must be **25 words or fewer**. Rewrite if necessary.
- **OPERATIONAL**: Each theme_statement must describe **clear conceptual boundaries** suitable for consistent use in coding.

────────────────────────────────────────
PROCESS
────────────────────────────────────────
1) Identify the **dominant shared idea(s)** in the cluster.  
   • If one idea clearly dominates → return ONE theme_statement.  
   • If multiple distinct ideas are well supported → return multiple theme_statements.
2) For each idea:
   • Write a theme_statement (≤25 words) that is atomic, grounded, and operational.
3) Check each theme_statement for atomicity and length compliance.  
   • If non-compliant, rewrite until it satisfies all criteria.

────────────────────────────────────────
STATEMENT RULES (language-agnostic)
────────────────────────────────────────
To enforce atomicity, each theme_statement must:
- **Not contain coordinating conjunctions or list markers** in {language}.
- **Avoid compound punctuation**, including: "/", "&", ",", ";", ":", "-", "–"  
  (Hyphens are allowed only within lexicalized single words, e.g., "self-service").
- **Contain only one main action (verb)**  
  • Allowed: "reduce waiting time"  
  • Not allowed: "reduce waiting time and improve service quality"

**After writing each theme_statement, perform a self-check**:  
If any forbidden patterns are present, rewrite until all rules are satisfied.
...
You are a **{language} qualitative analyst** specialized in matching descriptive codes from a codebook to themes identified in response patterns to an open-ended survey question.
...
GOAL
────────────────────────────────────────
Return **all existing codes** from the codebook that are clearly relevant to describing the presented theme(s).
...
2) Match on **semantic meaning**, not just word overlap:
   – Focus on **meaning**, **scope**, and **level of abstraction** in light of the <survey_question>.
   – Ignore superficial or surface-level matches.

3) Include only codes with substantial conceptual overlap:
   – Return all codes that meaningfully and directly reflect the theme.
...
- Do **not** create new codes or modify any existing ones.
...
DECISION RULES
────────────────────────────────────────
Step 1: Check abstraction alignment
- If the theme_statement is at the **same abstraction level** as an existing code:  
    → Apply USE, MODIFY, or CREATE depending on coverage threshold.  
- If the theme_statement is **more specific** (lower abstraction) than an existing broader code:  
    → CREATE a new code, unless the existing code already captures this exact subtheme.  
- If the theme_statement is **broader** (higher abstraction) than all existing codes:  
    → CREATE a new broader code only if needed for parsimony.

Step 2: Apply coverage thresholds (within the same abstraction level)
- USE if an existing code covers ≥90% of the theme’s meaning.  
- MODIFY if an existing code covers 60–89%.  
- CREATE if no existing code covers ≥60%.  
...
────────────────────────────────────────
CONSTRAINTS for code labels and definitions (existing, modified, or new)
────────────────────────────────────────
- Label: short phrase ≤10 words.  
- Definition: ≤25 words, operational, observable, grounded in actual responses.  
...
{{
  "coding_decisions": [
    {{
      ...
      "justification": "explicitly state abstraction relationship + coverage rule applied"
    }},
    {{
      "theme_number": 2,
      "decision": "...",
...
- Every decision must include a justification referencing abstraction + coverage.  
- Each theme_statement must map to exactly one decision.  
"""


VALIDATION_PROMPT = """
You are a **{language} qualitative data analyst** specializing in codebook validation.  
You will review your colleague’s coding recommendations for a cluster that contains one or more themes derived from survey responses.  
...
GOAL
────────────────────────────────────────
Review <coding_recommendation> by either approving or rejecting the recommendations.
...
When REJECT, the decision_rationale must clearly state whether the recommendation was **substituted** or **refined**, and why.
...
REVIEW CRITERIA
────────────────────────────────────────
Base your decision on evaluating EACH theme/code recommendation:
- **Semantic fit** — Does the code capture the theme’s meaning and scope?  
- **Atomicity** — Do the code and definition capture EXACTLY one single idea, without compounding sub-ideas or related themes?  
- **Parsimony** — Is reuse of existing codes maximized where possible?  
- **Redundancy** — Is overlap with existing codes avoided?  
...
- Use exact existing names/definitions when substituting with an existing code.  
